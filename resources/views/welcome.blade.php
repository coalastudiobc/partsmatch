    ...
    <div class="container">
        ...
        <div class="collection-main">
            <div class="collect-tab-btns">
                ...
                <div>
                    <div class="slick-tab ">
                        @foreach ($subcategories as $subcategory)
                            <a href="javascript:void(0)" data-url="{{ route('welcome.index', $subcategory->id) }}"
                                sub-id="{{ $subcategory->id }}"
                                class="tab-inner-box productsubcategory {{ url()->current() == route('welcome.index', $subcategory->id) ? 'active' : '' }}">
                                {{ $subcategory->name }}
                            </a>
                        @endforeach
                    </div>
                ...
                <a href="{{ route('dealer.products.interior') }}" class="btn secondary-btn view-btn interiorIndex">
                    View All
            ...
            <div class="heading-with-tab">
                ...
                <div class="heading-tab">
                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                        @foreach ($subcategories as $subcategory)
                            <li class="nav-item" role="presentation">
                                <button href="javascript:void(0)"
                                    data-url="{{ route('welcome.index', $subcategory->id) }}"
                                    class="nav-link navButton productsubcategory {{ url()->current() == route('welcome.index', $subcategory->id) ? 'active' : '' }}"
                        ...
                        <li class="nav-item" role="presentation">
                            <a href="{{ route('dealer.products.interior') }}" class="nav-link" id="contact-tab">
                                View All
...
<script>
    ...
    $('.productsubcategory').on('click', function() {
        ...
        suburl = $(this).attr('sub-id');
        url2 = APP_URL + '/dealer/products/interior/' + suburl;

        jQuery('.interiorIndex').attr('href', url2);
        url = $(this).attr('data-url');
        // jQuery('.interiorIndex').attr('href', url);
        ...
        function handleStateData(response) {
            if (response.success == true) {
                console.log('hererererer')
                jQuery('.productIndex').html(response.homeProducts)
                jQuery('.tabProduct').html(response.homerandomProducts)
                // location.reload();
                // return toastr.success(response.msg);
            } else {
                jQuery('#errormessage').html(response.error);
